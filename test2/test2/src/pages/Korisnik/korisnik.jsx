  ...
  return (
    <div className="user-profile">
      ...
      <div>
        ...
        <Link to="/uredjivanjeKorisnika" style={{ marginBottom: '20px', display: 'inline-block' }}>
          <button>Uredi profil</button>
        </Link>

        <label><br />Tip korisnika:</label> <span>{user.tipKorisnika}</span><br />
        <label>Datum registracije:</label> <span>{user.datumRegistracije}</span><br />
        <label>Korisničko ime:</label> <span>{user.username}</span><br />
        <label>Email:</label> <span>{user.mail}</span><br />
        <label>Ime:</label> <span>{user.ime}</span><br />
        <label>Prezime:</label> <span>{user.prezime}</span><br />
        <label>Datum rođenja:</label> <span>{user.datumRodjenja}</span><br />
        <label>Pol:</label> <span>{user.pol}</span><br />
        <label>Telefon:</label> <span>{user.telefon}</span><br /><br />

        <label>Osnovne informacije (Bio):</label><br />
        <p>{user.bio}</p>
      </div>
      ...
      <ul>
        {user.oruzja?.length > 0 ? (
          user.oruzja.map((oruzje, index) => (
            <li key={index}>{oruzje}</li>
          ))
        ) : (
          <li>Nema podataka</li>
        )}
